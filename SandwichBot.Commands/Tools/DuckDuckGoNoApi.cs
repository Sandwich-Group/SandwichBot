...
namespace HoLLy.DiscordBot.Sandwich.Tools
{
    public static class DuckDuckGoNoApi
    {
        ...
        public static Uri SearchImage(string query)
        {
            var wc = new WebClient();
            
            var searchPage = wc.DownloadString(ApiBase + $"?q={HttpUtility.UrlEncode(query)}&t=h_&iax=images&ia=images");
            var vdq = RegexVqdToken.Match(searchPage).Groups[1].Value;

            var searchResult = wc.DownloadString(ApiBase + "i.js" + $"?l=wt-wt&o=json&q=box&vqd={vdq}&f=,&p=1");
